...
import com.app.creaseart.adapter.AdapterPickUpAfterConvertedBookings;
...
public class FragementPickUpAfterConvertedBooking extends Fragment implements OnCustomItemClicListener, ApiResponse {

    private RecyclerView recycler_service;
    private ArrayList<ModelCategory> arrayList;
    private AdapterPickUpAfterConvertedBookings adapterPickUpAfterConvertedBookings;
    ...
    private void getData() {
        ...
        if (AppUtils.isNetworkAvailable(mActivity)) {
            ...
            String url = JsonApiHelper.BASEURL + JsonApiHelper.PICKUPBOYAFTERCONVERTEDLEAD + "user_id=" + AppUtils.getUserId(mActivity);
            new CommonAsyncTaskHashmap(1, mActivity, this).getqueryNoProgress(url);
            ...
            if (method == 1) {
                ...
                if (commandResult.getString("success").equalsIgnoreCase("1")) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        serviceDetail.setStickerCode(jo.getString("StickerCode"));

                        arrayList.add(serviceDetail);
                    }
                    adapterPickUpAfterConvertedBookings = new AdapterPickUpAfterConvertedBookings(mActivity, FragementPickUpAfterConvertedBooking.this, arrayList);
                    recycler_service.setAdapter(adapterPickUpAfterConvertedBookings);
                    ...
                    arrayList.clear();
                    if (adapterPickUpAfterConvertedBookings != null) {
                        adapterPickUpAfterConvertedBookings.notifyDataSetChanged();
                    }
                    if (swipeRefreshLayout != null) {
                        swipeRefreshLayout.setRefreshing(false);
